...
const Profile = () => {
  ...
  const handleSignOutConfirm = async () => {
    try {
      await signOut();
      // Clear SecureStore
      const keysToDelete = ["__clerk_client_jwt"];
      await Promise.all(
        keysToDelete.map((key) => SecureStore.deleteItemAsync(key))
      );
      console.log(`deleted ${keysToDelete[0]}`);
  ...
  return (
    <SafeAreaView className="flex-1 bg-black">
      ...
      >
        ...
        <View className="flex flex-col items-start justify-center bg-black rounded-lg px-5 py-3">
          <View className="flex flex-col items-start justify-start w-full">
            <InputField
              label="First name"
              placeholder={user?.firstName || "Not Found"}
              containerStyle="w-full bg-stone-800"
              inputStyle="p-3.5 text-white" // Set text color to black
              editable={false}
            />
